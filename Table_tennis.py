        ...
        for i in range(16):
            ...
            if self.roll_in[i] == 0:
                ...
                if (
                    ...
                    rot_norm = rot.norm()
                    if (
                        new_rot.norm() > self.friction_coeff_rotation * delta_t
                        and rot_norm > 0
                    ):
                        new_rot -= (
                            rot / rot_norm * self.friction_coeff_rotation * delta_t
                        )
                    else:
                        new_rot == ti.Vector([0.0, 0.0, 0.0])

                    self.ball.angle[i] = new_angle
                    if new_vel.norm() < self.friction_coeff_ball_table * delta_t * 2:
                        self.ball.vel[i] = ti.Vector([0.0, 0.0, 0.0])
                    else:
                        self.ball.vel[i] = new_vel

                    if new_rot.norm() < self.friction_coeff_rotation * delta_t * 2:
                        self.ball.rot[i] = ti.Vector([0.0, 0.0, 0.0])
                    else:
